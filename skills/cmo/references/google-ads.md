...
Google Ads 自动化投放技能：覆盖 **YouTube 视频广告**（由 video MDX 驱动）和 **Google Search SEM 关键词广告**（由 Buda GTM ICP 数据驱动）。
...
| 投放线 | 配置来源 | 创建脚本 | 暂停脚本 |
|--------|---------|---------|---------|
| Search（SEM） | `apps/buda/src/domains/gtm/data/icps/<icp>.ts` 的 `sem` 块 | `create-google-sem-campaign.mjs` | `pause-google-sem-campaign.mjs` |
| Video（YouTube） | `apps/buda/content/videos/<lang>/*.mdx` 的 `ads` frontmatter | `create-google-video-campaign.mjs` | `pause-google-video-campaign.mjs` |
...
node .agents/skills/cmo/scripts/create-google-sem-campaign.mjs
...
`.agents/skills/cmo/app/.data/google-sem-state.json`
...
再次运行 `create-google-sem-campaign.mjs --icp seo-marketer` 会直接跳过，除非 `--force`。
...
node .agents/skills/cmo/scripts/pause-google-video-campaign.mjs \
  --mdx apps/buda/content/videos/zh-CN/buda-intro-general.mdx
...
`--ads` 会针对每个有 `ads.enabled: true` 的 MDX 调用 `create-google-video-campaign.mjs`。
...
`create-google-video-campaign.mjs` 里用了 `customers/{customerId}/assets/{youtubeId}` 作为 video asset resource name。Google Ads 实际要求先通过 `assets.create` 创建 YouTube video asset 拿到真实 resource name。目前这一步是 **TODO**（代码里有注释）。真实投放前需要补完这个 asset 创建步骤，否则 `ads.create` 会报错。
...
| 行为 | SEM | Video |
|------|-----|-------|
| 重复运行不创建重复广告 | ✅ 看 `app/.data/google-sem-state.json` | ✅ 看 MDX 里的 `campaignId` |
| 用 `--force` 可以强制重建 | ✅ | ✅ |
...
**Q: ICP 里没有 `sem` 块会怎样？**
`create-google-sem-campaign.mjs` 会在校验阶段报错：`icp.sem.keywords must be a non-empty array`。去对应 ICP `.ts` 文件里补齐 `sem` 块即可。
...
└── app/.data/
    └── google-sem-state.json         # ignored SEM campaign mapping cache
```
