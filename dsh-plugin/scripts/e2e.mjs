...
if (missingEnvVars.length > 0 || !nodeOk) {
  console.log("Skipping live E2E — prerequisites not met:");
...
console.log(`Prerequisites satisfied (Node ${process.version}). Running live E2E…`);

try {
  await exec("corepack", ["pnpm", "exec", "vitest", "run", "src/e2e/live-scenario.test.ts"], {
    cwd: root,
    env: process.env,
